...
class TestVpos:
    ...
    def test_not_create_payment_if_customer_format_is_invalid(self):
        merchant = Vpos()
        payment = merchant.new_payment("99256301", "123.45")

        assert payment.get('status_code') == 400

    def test_not_create_a_new_payment_if_amount_format_is_invalid(self):
        merchant = Vpos()
    ...
    def test_not_create_new_refund_if_parent_transaction_id_is_blank(self):
        merchant = Vpos()
        response = merchant.new_refund(None)

        assert response.get('status_code') == 400

    def test__not_create_new_refund_if_supervisor_card_is_invalid(self):
        merchant = Vpos()
